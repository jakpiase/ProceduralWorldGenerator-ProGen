...
void StochasticBinarySpacePartitioningLevelGenerator::generate_room(const BoundingBox& bounding_box, Scene& scene) {
    auto& random_generator = scene.get_random_generator();

    DLOG(INFO) << "Final bounding box: " << bounding_box;

    float room_width = minimal_room_size + random_generator.random(static_cast<int>(bounding_box.get_width() - 2 * bounding_box_padding - 2 * minimal_room_size));
    float room_height = minimal_room_size + random_generator.random(static_cast<int>(bounding_box.get_height() - 2 * bounding_box_padding - 2 * minimal_room_size));
    ...
    SimpleRoomGenerator room_generator(BoundingBox(Point(room_x, room_y), Point(room_x + room_width, room_y + room_height)));
...
std::pair<BoundingBox, BoundingBox> StochasticBinarySpacePartitioningLevelGenerator::calculate_new_bounding_boxes(const BoundingBox& bounding_box, RandomNumberGenerator* random_generator) {
    ...
    bool horizontal = random_generator->random_bool();
...
void StochasticBinarySpacePartitioningLevelGenerator::split_space(const BoundingBox& bounding_box, Scene& scene) {
    auto& random_generator = scene.get_random_generator();
