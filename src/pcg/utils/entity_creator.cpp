...
void EntityCreatorImpl::handle_room_content_generation(Scene& scene, const BoundingBox2i& room_box) const {
    ...
    parse_room_content_to_entities(scene, room, room_box);
//    for (size_t i = 0; i < room.size(); ++i) {
//        for (size_t j = 0; j < room[0].size(); ++j) {
//            if (room[i][j] != RoomContentType::EMPTY) {
//                Entity entity = scene.create_entity();
...
void
EntityCreatorImpl::parse_room_content_to_entities(Scene& scene, const Room& room, const BoundingBox2i& room_box) const {
